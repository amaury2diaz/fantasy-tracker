...
export class LigaComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    if (this.currentSearch) {
      this.ligaService
        ...
        .subscribe(
          (res: HttpResponse<ILiga[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
          () => this.onError()
        );
      return;
    }
  ...
  trackId(index: number, item: ILiga): number {
    // eslint-disable-next-line @typescript-eslint/no-unnecessary-type-assertion
    return item.id!;
  }

  registerChangeInLigas(): void {
    this.eventSubscriber = this.eventManager.subscribe('ligaListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: ILiga[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
